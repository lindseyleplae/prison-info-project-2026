---
title: "Norton Correctional Facility"
slug: norton
state: ks
system: "KDOC"
population: men
summary: "A medium- and minimum-security state prison for men in Norton, in far-northwest Kansas, that also oversees a minimum-custody satellite, the NCF–East Unit in Stockton."
aliases: ["NCF", "Norton Correctional Facility"]
city: "Norton"
county: "Norton"
address: "11130 Road E4, Norton, KS 67654-0546"
securityLevel: "Medium and minimum security (men)"
facilityType: "State Prison"
contact:
  mainPhone: "(785) 877-3389"
  mailingAddress: "11130 Road E4, Norton, KS 67654-0546"
providers:
  phone: "ICSolutions"
  video: "ICSolutions"
visiting:
  days: "Set by the facility"
  hours: "Confirm the facility's current visiting schedule"
  scheduling: "Every visitor must be on the resident's approved list first; visits are reserved statewide through ICSolutions (icsolutions.com) at least 72 hours ahead. Confirm with the facility."
  contactVisits: true
links:
  official: "https://www.doc.ks.gov/facilities/ncf"
  inmateLocator: "https://kdocrepository.doc.ks.gov/kasper/search/disclaimer"
sources:
  - name: "KDOC — Norton Correctional Facility"
    url: "https://www.doc.ks.gov/facilities/ncf"
  - name: "KDOC — Visitation (In-Person)"
    url: "https://www.doc.ks.gov/visitation-in-person"
lastVerified: 2026-06-16
reviewBy: 2026-08-18
---

:::callout{variant="warning"}
Norton Correctional Facility is a medium- and minimum-security prison for men in far-northwest Kansas. It also oversees a separate minimum-custody satellite, the NCF–East Unit in Stockton, about 50 miles to the southeast — confirm which location holds the person before traveling. Check the [Kansas Adult Supervised Population Electronic Repository (KASPER)](https://kdocrepository.doc.ks.gov/kasper/search/disclaimer) before traveling or sending anything, because people are transferred without family notification. Personal mail goes directly to the facility and may include only letters and photographs (see [Mail](#mail)).
:::

:::key-info
- **Location:** Norton, Norton County, in far-northwest Kansas
- **Population:** Men, medium and minimum custody
- **Notable:** A far-northwest-Kansas men's prison that also oversees a minimum-custody satellite, the NCF–East Unit in Stockton
- **Visitation:** By approved list, reserved statewide through ICSolutions at least 72 hours ahead
:::

## Overview

Norton Correctional Facility is a state prison for men operated by the Kansas Department of Corrections. It is in Norton, the seat of Norton County, in the far northwest corner of Kansas near the Nebraska state line. The facility holds men in medium and minimum custody.

Kansas DOC assigns custody level based on factors that include sentence length, time remaining, and conduct, and a person's custody level can change during incarceration. The custody class and housing unit held at a given prison affect visiting arrangements, so families confirm the details that apply to the specific person before a first visit.

## What Makes the Norton Correctional Facility Different

Norton Correctional Facility sits in the far northwest corner of Kansas, one of the most remote of the state's prisons, near the Nebraska state line and a long drive from the larger population centers in the eastern part of the state. Families planning a visit account for that distance and for limited services in the immediate area.

The facility also oversees a satellite minimum-custody unit, the NCF–East Unit, in Stockton, in Rooks County about 50 miles to the southeast. The East Unit operates under Norton's administration and is not a separate facility with its own page. Because a person may be assigned to either the main facility in Norton or the East Unit in Stockton, and the two sites are about an hour apart by road, families confirm the exact location where the person is held before traveling or sending mail. The East Unit is at 1210 S. Cedar Street, Stockton, KS 67669.

## Visiting

The statewide KDOC rules above — the approved visitor list, the dress code, ID, and item limits — apply at the Norton Correctional Facility. The facility's own arrangements:

:::key-info
- **Approved list first:** Before any visit, each visitor must be on the resident's pre-approved visitation list. The number of visitors per resident is four.
- **Scheduling:** All visits are reserved statewide through ICSolutions at icsolutions.com, at least 72 hours before visitation begins for that week. Visitors who are not online can contact the facility's visitation clerk.
- **Days and hours:** Set by the facility. Kansas adult facilities generally offer two slots per day, one morning and one afternoon; a visitor may reserve one or two slots on a day but cannot book multiple days in one weekend. Confirm the current schedule with the facility.
- **Contact visits:** Appropriate contact, such as a brief embrace, is allowed between a resident and a visitor.
- **Location matters:** Confirm whether the person is held at the main facility in Norton or the NCF–East Unit in Stockton before scheduling, as the two sites are about 50 miles apart.
- **Main line:** (785) 877-3389
:::

## Getting There and Parking

Norton Correctional Facility is at 11130 Road E4 in Norton, in Norton County in far-northwest Kansas. Norton sits along US Highway 36 near the Nebraska state line; US Highway 283 runs north–south through the area. The drive from the eastern part of the state is long, and the nearest larger commercial airports are a considerable distance away. The NCF–East Unit is in Stockton, about 50 miles southeast by way of US Highway 283.

Parking is on site. Visitors confirm current entry procedures, the visitor-processing location, and what may be brought onto the grounds with the facility before arriving, because electronic devices and personal items are generally not permitted inside.

## Nearby Services

Norton has limited lodging and dining, with a small range of motels, restaurants, and stores serving the surrounding area. Stockton, near the East Unit, is also a small town with limited services. Emergency medical care is available locally, with larger hospitals a longer drive away in the region. Visitors traveling a long distance generally plan ahead for fuel, food, and overnight stays, as options between towns in this part of Kansas are sparse.

## Mail

Kansas sends mail straight to the facility — there is no off-site scanning vendor. The Kansas Department of Corrections accepts only letters and photographs through the mail; it will not accept other items mailed into a facility. Every envelope must show the resident's full name and KDOC registration number, plus the sender's name and return address — mail missing this information is not delivered. Incoming and outgoing mail that is not legal, official, or privileged may be inspected or read.

Address personal mail to the resident at the Norton Correctional Facility, 11130 Road E4, Norton, KS 67654-0546. Mail for a person held at the satellite unit goes to the NCF–East Unit, 1210 S. Cedar Street, Stockton, KS 67669 — confirm the correct location before sending. Legal, official, and privileged mail goes to the facility and is handled separately. Books, publications, and packages are not sent as ordinary letter mail; confirm the current approved-vendor and publisher-direct rules with the facility before ordering.

Mail rules are set by Kansas Administrative Regulation 44-12-601 and KDOC policy IMPP 12-120.

## Learn More

- [Visiting someone in a Kansas prison](/states/kansas/guides/visiting/) — the approved visitor list, scheduling visits through ICSolutions, days and hours, and what to expect at the facility.
- [Sending mail to a Kansas prison](/states/kansas/guides/mail/) — what the Kansas DOC accepts, how to address an envelope, and the rules for legal mail and publications.
- [Phone calls and video visits in Kansas](/states/kansas/guides/phone-video/) — setting up phone service through ICSolutions, video visits, and electronic messaging through GettingOut.
- [Sending money in Kansas](/states/kansas/guides/money/) — depositing to a resident's trust account through Access Corrections, by app, online, phone, retail, or mailed money order.
- [Medical care in Kansas prisons](/states/kansas/guides/medical/) — how health care is provided, requesting care, and what families can and cannot do.
- [Intake and transfers in Kansas](/states/kansas/guides/transfers/) — reception at El Dorado (men) and Topeka (women), and how people move between facilities.
