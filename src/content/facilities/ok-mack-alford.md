---
title: "Mack Alford Correctional Center"
slug: mack-alford
state: ok
system: "Oklahoma DOC"
population: men
summary: "A medium-security state prison for men in Atoka County, Oklahoma — listed by the state at Atoka, with a street address near Stringtown."
aliases: ["MACC", "Mack Alford", "Mack Alford Correctional Center"]
city: "Atoka"
county: "Atoka"
address: "13001 North Highway 69, Atoka, OK 74525"
securityLevel: "Medium security (men)"
facilityType: "State Prison"
contact:
  mainPhone: "580-346-7301"
  mailingAddress: "13001 North Highway 69, Atoka, OK 74525"
providers:
  phone: "Securus"
  video: "Securus"
visiting:
  days: "Saturday and Sunday (additional weekdays vary)"
  hours: "8 a.m. – 4 p.m."
  scheduling: "Every visitor must be approved on the incarcerated person's list first; all visits are scheduled in advance through the statewide Visitation Unit (405-768-3269) — no walk-ins, requests by Wednesday 1 p.m."
  contactVisits: true
links:
  official: "https://oklahoma.gov/doc/facilities.html"
  inmateLocator: "https://okoffender.doc.ok.gov/"
sources:
  - name: "Oklahoma DOC — Facilities"
    url: "https://oklahoma.gov/doc/facilities.html"
  - name: "Oklahoma DOC — Visitation"
    url: "https://oklahoma.gov/doc/facilities/visitation.html"
lastVerified: 2026-06-16
reviewBy: 2026-09-20
---

:::callout{variant="warning"}
Mack Alford Correctional Center is a medium-security prison for men. Atoka County has two separate Oklahoma DOC prisons — Mack Alford Correctional Center (medium security) and the Howard McLeod Correctional Center (a separate minimum-security men's prison) — so confirm which one holds the person on the [Oklahoma DOC offender lookup](https://okoffender.doc.ok.gov/) before traveling or sending anything. People are also transferred without family notification. Personal mail does not go to the prison; it goes off-site to a Securus Digital Mail Center for scanning (see [Mail](#mail)).
:::

:::key-info
- **Location:** Atoka County, in southeastern Oklahoma; the state lists the facility at Atoka, and its street address is near Stringtown
- **Population:** Men, medium custody
- **Notable:** One of two Oklahoma DOC prisons in Atoka County — distinct from the minimum-security Howard McLeod Correctional Center, also in the county
- **Visitation:** By approved list, scheduled through the statewide Visitation Unit; no walk-ins
:::

## Overview

Mack Alford Correctional Center is a state prison for men operated by the Oklahoma Department of Corrections. It is in Atoka County, in southeastern Oklahoma; the Oklahoma DOC facilities page lists the facility at Atoka, while its street address is near the town of Stringtown. The facility houses men in medium custody.

Oklahoma DOC assigns custody level based on factors that include sentence length, time remaining, and conduct, and a person's custody level can change during incarceration. The custody class and housing unit held at a given prison determine whether visits are contact or non-contact, so families confirm the arrangement that applies to the specific person before a first visit.

## What Makes Mack Alford Correctional Center Different

Mack Alford Correctional Center is a medium-security prison, and it is one of two Oklahoma DOC facilities in Atoka County. The other is the Howard McLeod Correctional Center, a separate minimum-security prison for men, also located in the county. The two are different prisons with different security levels and different addresses, so a person, a court document, or an older directory that references an "Atoka County" prison may be pointing to either one. Families confirm which facility holds the person on the Oklahoma DOC offender lookup before traveling or sending mail. The Oklahoma DOC facilities page lists Mack Alford at Atoka, while the facility's street address is near Stringtown — both places are in Atoka County.

## Visiting

The statewide Oklahoma DOC rules above — the approved visitor list, the dress code, ID, and item limits — apply at Mack Alford Correctional Center. The facility's own arrangements:

:::key-info
- **Approved list first:** Before any visit, each visitor must be on the incarcerated person's approved visitor list. Submit a Visitor Application Form (online or by mail to the Visitation Unit, P.O. Box 11400, Oklahoma City, OK 73136-0400) and pass a background check (required for ages 18 and older); approval can take up to 90 days.
- **Scheduling:** Through the statewide Visitation Unit at 405-768-3269. All visits must be scheduled in advance — there are no walk-ins — with requests received by Wednesday at 1 p.m. for that period.
- **Days and hours:** Saturday and Sunday are available, with additional weekdays varying; sessions run 8 a.m. to 4 p.m. Whether a visit is contact or non-contact depends on the person's security level and housing unit.
- **Main line:** 580-346-7301
:::

## Getting There and Parking

Mack Alford Correctional Center is at 13001 North Highway 69 in Atoka County, in southeastern Oklahoma, near Stringtown. The facility is along US Highway 69, between Atoka to the south and McAlester to the north. The nearest large commercial airports are in the Oklahoma City area to the northwest and the Dallas–Fort Worth area to the south.

Parking is on site. Visitors confirm current entry procedures, the visitor-processing location, and what may be brought onto the grounds with the facility before arriving, because electronic devices and personal items are not permitted inside — visitors may generally bring only identification, a car key, and coins for vending machines.

## Nearby Services

The Atoka and Stringtown area has limited lodging and dining, with additional options in nearby Atoka and McAlester. Emergency medical care is available locally, with larger hospitals in the McAlester area to the north. Visitors traveling a long distance generally find the most options for fuel, food, and overnight stays along the US Highway 69 corridor and in McAlester.

## Mail

Oklahoma DOC routes incoming **personal mail** through an off-site digital vendor. As of September 1, 2024, letters and photos are mailed to the Securus Digital Mail Center in Dallas, where they are scanned and delivered to the inmate as a digital color image on a Securus tablet or kiosk — the original is not delivered. Inmates without device access get a free black-and-white printout. Address personal mail exactly as:

> Inmate's Full Name + ODOC Number
> Securus Digital Mail Center - Okla. Dept. of Corrections
> P.O. Box 223566
> Dallas, TX 75222-3566

Illegible or incorrectly addressed mail may be returned or delayed.

**Legal mail and special correspondence** are *not* sent to the digital center — they go directly to the facility and are handled under the rules for privileged mail. **Publications** (magazines, newspapers, books) also go to the facility and must come directly from the publisher or an approved vendor. **Packages** cannot be sent to the digital mail center and must go to the facility.

## Learn More

- [Visiting in Oklahoma](/states/oklahoma/guides/visiting/) — the approved visitor list, scheduling through the Visitation Unit, dress code, and what you can bring.
- [Mail in Oklahoma](/states/oklahoma/guides/mail/) — how Oklahoma's digital mail scanning works and where to send letters, legal mail, and publications.
- [Phone & Video in Oklahoma](/states/oklahoma/guides/phone-video/) — setting up calls, tablets, and video visits through Securus.
- [Sending Money in Oklahoma](/states/oklahoma/guides/money/) — depositing to a trust account through JPay.
- [Medical Care in Oklahoma](/states/oklahoma/guides/medical/) — how health care works in Oklahoma DOC facilities.
- [Transfers & Reception in Oklahoma](/states/oklahoma/guides/transfers/) — intake at LARC (men) and Mabel Bassett (women) and how people move between facilities.
