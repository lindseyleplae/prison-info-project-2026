---
title: "Blackburn Correctional Complex"
slug: blackburn-correctional-complex
state: ky
system: "KY DOC"
population: men
summary: "A minimum-security state prison for men in Lexington, Kentucky — the Kentucky Department of Corrections' largest minimum-security men's institution."
aliases: ["BCC", "Blackburn Correctional Complex"]
city: "Lexington"
county: "Fayette"
address: "3111 Spurr Road, Lexington, KY 40511"
securityLevel: "Minimum security (men)"
facilityType: "State Prison"
contact:
  mainPhone: "859-246-2366"
  mailingAddress: "3111 Spurr Road, Lexington, KY 40511"
providers:
  phone: "Securus"
visiting:
  days: "Set by the facility"
  hours: "Confirm the facility's current visiting schedule"
  scheduling: "Every visitor must be on the inmate's approved visiting list first; visits are arranged with the facility (no statewide scheduler). Confirm days and hours with the facility."
  contactVisits: true
links:
  official: "https://corrections.ky.gov/Facilities/AI/BCC/Pages/default.aspx"
  inmateLocator: "https://kool.corrections.ky.gov/"
sources:
  - name: "KY DOC — Blackburn Correctional Complex"
    url: "https://corrections.ky.gov/Facilities/AI/BCC/Pages/default.aspx"
  - name: "KY DOC — Adult Institutions"
    url: "https://corrections.ky.gov/facilities/ai/pages/default.aspx"
lastVerified: 2026-06-16
reviewBy: 2026-08-19
---

:::callout{variant="warning"}
Blackburn Correctional Complex is a minimum-security prison for men. Confirm the person's current facility on the [Kentucky Offender Online Lookup](https://kool.corrections.ky.gov/) before traveling or sending anything — people are transferred between facilities, and a large share of Kentucky's state-sentenced people are held in county jails rather than state prisons, each with its own rules. Incoming personal mail goes directly to the facility at the address below (see [Mail](#mail)).
:::

:::key-info
- **Location:** Lexington, Fayette County, in central Kentucky
- **Population:** Men, minimum custody
- **Notable:** The Kentucky Department of Corrections' largest minimum-security men's institution
- **Visitation:** By approved visiting list, arranged with the facility; confirm days and hours before traveling
:::

## Overview

Blackburn Correctional Complex is a state prison for men operated by the Kentucky Department of Corrections. It is in Lexington, the seat of Fayette County, in central Kentucky. The facility houses men in minimum custody.

Kentucky DOC assigns custody level based on factors that include sentence length, time remaining, and conduct, and a person's custody level can change during incarceration. The custody class and housing assignment held at a given prison affect visiting arrangements, so families confirm the arrangement that applies to the specific person before a first visit.

## What Makes the Blackburn Correctional Complex Different

Blackburn Correctional Complex is the Kentucky Department of Corrections' largest minimum-security institution for men. It sits on the north side of Lexington, in Fayette County, which places it within easy reach of central Kentucky's largest population center. As a minimum-security facility, it holds men in the lowest custody class in the state system; the kinds of programming, work assignments, and movement available at a minimum-security prison differ from those at the state's medium- and maximum-security institutions.

## Visiting

The statewide KY DOC rules above — the approved visitor list, the dress code, ID, and item limits — apply at Blackburn Correctional Complex. The facility's own arrangements:

:::key-info
- **Approved list first:** Before any visit, each visitor must be on the inmate's approved visiting list. The list is requested through the incarcerated person and is subject to approval; all adult visitors need valid government-issued photo identification, and minors must be accompanied by a parent or legal guardian.
- **Scheduling:** Visits are arranged with the facility. Kentucky has no statewide online visit scheduler, and days and hours are set facility by facility, so confirm the current schedule with Blackburn Correctional Complex before traveling.
- **Days and hours:** Set by the facility. Confirm the current visiting schedule directly with the prison.
- **Main line:** 859-246-2366
:::

## Getting There and Parking

Blackburn Correctional Complex is at 3111 Spurr Road on the north side of Lexington, in Fayette County in central Kentucky. Lexington sits at the junction of Interstates 64 and 75 and is served by Blue Grass Airport to the southwest of the city.

Parking is on site. Visitors confirm current entry procedures, the visitor-processing location, and what may be brought onto the grounds with the facility before arriving, because electronic devices and personal items are generally not permitted inside — visitors may usually bring only identification, a car key, and coins for vending machines.

## Nearby Services

Lexington is a major city with a wide range of lodging, dining, fuel, and other services, including options near the Interstates 64 and 75 corridors. Emergency and hospital medical care is available in the Lexington area. Visitors traveling a long distance generally find the most choices for overnight stays and meals within the city.

## Mail

Incoming personal mail goes directly to the institution, addressed to the person by their committed name and Kentucky DOC inmate number, followed by the facility's mailing address. Mail is opened and inspected for contraband before delivery; under Kentucky Corrections policy, correspondence is delivered to the person within 48 hours of receipt on normal workdays.

Legal and other privileged mail — from a licensed attorney, a court, a government official, the Department of Public Advocacy, or Corrections officials — is opened only in the person's presence and should be clearly marked as legal mail.

Books and magazines must be shipped new, directly from a publisher or an approved retailer; items sent by individuals are refused. Packages are limited to approved-vendor care packages arranged through the facility.

Verify the exact mailing address, current mail rules, and approved package vendor with the facility before sending anything, because procedures change.

## Learn More

- [Visiting a Kentucky prison](/states/kentucky/guides/visiting/) — approved visitor lists, scheduling, dress code, and what to expect at the gate.
- [Sending mail in Kentucky](/states/kentucky/guides/mail/) — how to address mail, what is allowed, and how books and packages are handled.
- [Phone calls and video visits](/states/kentucky/guides/phone-video/) — setting up calls, video visits, and messaging with someone in a Kentucky prison.
- [Sending money](/states/kentucky/guides/money/) — how to deposit funds to a person's account and what the money can be used for.
- [Medical care](/states/kentucky/guides/medical/) — how health care works in Kentucky prisons and how to raise a medical concern.
- [Intake, classification, and transfers](/states/kentucky/guides/transfers/) — where people enter the system, how they are classified, and why some state inmates are held in county jails.
