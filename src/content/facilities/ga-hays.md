---
title: "Hays State Prison"
slug: hays
state: ga
system: GDC
population: men
summary: "A close-security state prison for men in Trion, in northwest Georgia — one of the state's higher-security men's prisons."
aliases:
  - Hays
  - Hays State Prison
city: Trion
county: Chattooga
address: "777 Underwood Drive, Trion, GA 30753"
securityLevel: "Close security (men)"
facilityType: "State Prison"
contact:
  mainPhone: "(706) 857-0400"
  mailingAddress: "P.O. Box 668, 777 Underwood Drive, Trion, GA 30753"
providers:
  phone: "Securus"
visiting:
  days: "Saturday, Sunday, and state holidays"
  hours: "Sat-Sun and state holidays, 9:00 AM - 3:00 PM"
  scheduling: "By appointment through GDC's online visitation portal; the visitor must first be approved on the person's list. As a higher-security prison, visiting may be more restricted — confirm the facility's current rules."
  contactVisits: true
links:
  official: "https://gdc.georgia.gov/locations/hays-state-prison"
  inmateLocator: "https://gdc.georgia.gov/offender-info/find-offender"
sources:
  - name: "GDC — Hays State Prison"
    url: "https://gdc.georgia.gov/locations/hays-state-prison"
  - name: "GDC — Visitation"
    url: "https://gdc.georgia.gov/friends-and-family/visit-inmate/visitation"
lastVerified: 2026-08-10
reviewBy: 2026-11-08
---

:::callout{variant="warning"}
**Hays State Prison is a close-security men's prison — one of Georgia's higher-security facilities.** As a higher-security prison, visiting may be more restricted, so confirm the facility's current rules and the person's eligibility before traveling. Confirm where the person is held on GDC's Find an Offender first. Personal mail goes to the facility with the person's GDC ID number (see Mail, below).
:::

:::key-info
**Location:** On Underwood Drive in Trion, Chattooga County, in northwest Georgia near the Tennessee/Alabama corner

**Population:** Men in close security — capacity about 1,101

**Notable:** A close-security prison that houses some of the state's higher-security men; includes a Faith and Character based dormitory and hosts the Paulding Residential Substance Abuse Treatment program

**Visitation:** Saturday, Sunday, and holidays by appointment; as a higher-security prison, visiting may be more restricted — confirm with the facility
:::

## Overview

Hays State Prison, on Underwood Drive in Trion in Chattooga County, is a **close-security state prison for men** in northwest Georgia, near the corner where Georgia meets Tennessee and Alabama. As a close-security facility, it houses some of the state's higher-security men, with a capacity of about **1,101**. The prison includes a Faith and Character based dormitory and serves as the host facility for the Paulding Residential Substance Abuse Treatment program.

## What Makes Hays State Prison Different

- **It is a close-security prison,** one of Georgia's higher-security men's facilities, so visiting may be more restricted than at lower-security prisons.
- **It houses some of the state's higher-security men.**
- **It includes a Faith and Character based dormitory** and hosts the Paulding Residential Substance Abuse Treatment program.
- **It is in northwest Georgia,** in Trion, near the Tennessee/Alabama corner and the Summerville and Rome areas.

## Visiting

The statewide GDC rules above — the approved visitor list, the dress code, ID, and item limits — apply at Hays State Prison. The facility's own arrangements:

:::key-info
**Days and hours:** Saturday, Sunday, and state holidays, 9:00 a.m. - 3:00 p.m.

**Higher-security prison:** As a close-security facility, visiting may be more restricted; confirm the facility's current rules and the person's eligibility

**Scheduling:** By appointment through GDC's online visitation portal; the visitor must first be approved on the person's list
:::

The full approval process is in [Visiting in Georgia](/states/georgia/guides/visiting/).

## Getting There and Parking

The prison is on Underwood Drive in Trion, Chattooga County, in northwest Georgia near the Tennessee/Alabama corner.

:::key-info
**By car:** In Trion, north of Summerville and northwest of Rome; reached from U.S. 27 through the Summerville and Trion area

**By transit:** There is no public transit to the prison; a private vehicle is required
:::

Distances are approximate, based on map routing. Visitor parking is on site.

## Nearby Services

Summerville and Trion have the closest gas and food; Rome, about an hour southwest, has the fullest range of lodging, food, and services. The nearest full-service hospital with a 24-hour emergency room is in the **Rome** area.

## Mail

Personal mail goes to the facility, addressed with the person's full name and **GDC ID number**. Georgia is moving toward centralized mail handling, so confirm the current mailing address before sending. Books and magazines must come from a publisher or established retailer, and legal mail is opened only in the person's presence. Full rules are in [Mail & Packages](/states/georgia/guides/mail/).

## Learn More

For detailed information about visiting and communicating with someone in a Georgia state prison:

- [Visiting in Georgia](/states/georgia/guides/visiting/) — the approved list, dress code, and the appointment portal
- [Mail & Packages](/states/georgia/guides/mail/) — addressing mail and the publisher-only rule
- [Phone & Video Calls](/states/georgia/guides/phone-video/) — Securus calls and tablet messaging
- [Sending Money](/states/georgia/guides/money/) — depositing through JPay
- [Medical & Mental Health](/states/georgia/guides/medical/) — the co-pay, grievances, and the Ombudsman
- [Transfers & Finding Someone](/states/georgia/guides/transfers/) — intake, permanent assignment, and the locator
