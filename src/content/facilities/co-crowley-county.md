---
title: "Crowley County Correctional Facility"
slug: crowley-county
state: co
system: CDOC
population: men
summary: "A privately operated (CoreCivic) medium-security prison in Olney Springs that holds Colorado state-sentenced men — the larger of Colorado's two private prisons."
aliases:
  - Crowley County
  - CCCF
  - Crowley County Correctional
city: Olney Springs
county: Crowley
address: "6564 State Highway 96, Olney Springs, CO 81062"
securityLevel: "Level III — medium (men); privately operated by CoreCivic"
facilityType: "Private Correctional Facility (CoreCivic)"
contact:
  mainPhone: "(719) 267-3548"
  visitingOffice: "(719) 267-3548"
  mailingAddress: "6564 State Highway 96, Olney Springs, CO 81062"
providers:
  phone: "Securus"
  video: "Securus"
visiting:
  days: "Scheduling window Thursday–Monday; CDOC posts the visiting day/hour schedule in a separate document — confirm the current days and hours with the facility"
  hours: "Not listed inline by CDOC — confirm with the facility"
  scheduling: "By appointment — call (719) 267-3548 ext. 23898 (Thu–Mon 8:30 a.m.–3:30 p.m.) or email crowleyvisitation@corecivic.com with the visitors' names, the person's name and CDOC number, and the requested date; an approved CDOC visiting application must be on file."
  contactVisits: true
links:
  official: "https://cdoc.colorado.gov/facilities/private-prisons/crowley-county-correctional-facility"
  inmateLocator: "https://www.doc.state.co.us/oss/"
sources:
  - name: "CDOC — Crowley County Correctional Facility (private prisons)"
    url: "https://cdoc.colorado.gov/facilities/private-prisons/crowley-county-correctional-facility"
  - name: "CoreCivic — Crowley County Correctional Facility"
    url: "https://www.corecivic.com/facilities/crowley-county-correctional-facility"
  - name: "CDOC — Contact an Inmate (mail and photocopy policy)"
    url: "https://cdoc.colorado.gov/resources-faq/contact-an-inmate"
  - name: "Arkansas Valley Regional Medical Center — Emergency (La Junta)"
    url: "https://www.avrmc.org/"
lastVerified: 2026-07-30
reviewBy: 2026-10-28
---

:::callout{variant="warning"}
**Crowley County is privately operated by CoreCivic under contract with the state, but the people held here are Colorado state inmates — so the statewide CDOC rules apply.** CDOC posts the visiting day-and-hour schedule in a separate document rather than inline, so confirm the current days and hours; schedule by calling (719) 267-3548 ext. 23898 (Thu–Mon) or emailing crowleyvisitation@corecivic.com, with an approved CDOC visiting application on file. Personal mail goes to the facility with the person's DOC number, and Crowley County photocopies incoming personal mail and destroys the original (see Mail, below).
:::

:::key-info
**Location:** On State Highway 96 in Olney Springs, Crowley County, in the Arkansas River valley

**Population:** Men in medium (Level III) custody, held for Colorado under a CoreCivic contract

**Notable:** The larger of Colorado's two private prisons; opened 1998; operated by CoreCivic

**Visitation:** By appointment; confirm the current days and hours with the facility
:::

## Overview

Crowley County Correctional Facility, on State Highway 96 in Olney Springs in the Arkansas River valley, is a **medium-security (Level III) prison operated by CoreCivic** under contract with the Colorado Department of Corrections. The people held here are **Colorado state-sentenced men**, so the statewide CDOC rules apply. It opened in **1998** and is the **larger of Colorado's two private prisons**, with programs in education (GED), vocational trades, mental health, substance abuse, and reentry.

## What Makes Crowley County Different

- **It is privately operated by CoreCivic,** though it holds Colorado state inmates under the statewide CDOC rules.
- **It is the larger of Colorado's two private prisons.**
- **CDOC posts its visiting schedule in a separate document,** so confirm the current days and hours before traveling.
- **It is in the Arkansas River valley,** about 54 miles east of Pueblo.

## Visiting

The statewide CDOC rules above — the approved list, the dress code, ID, and search rules — apply at Crowley County. The facility's own arrangements:

:::key-info
**Days and hours:** CDOC publishes the visiting day-and-hour schedule in a separate posted document rather than inline; the scheduling window is Thursday–Monday — confirm the current visiting days and hours with the facility

**Scheduling:** By appointment — call (719) 267-3548 ext. 23898 (Thu–Mon 8:30 a.m.–3:30 p.m.) or email crowleyvisitation@corecivic.com with the visitors' names, the person's name and CDOC number, and the requested date

**Application:** An approved CDOC visiting application must be on file (processed through CDOC's Private Prison Monitoring Unit)
:::

The full approval process is in [Visiting in Colorado](/states/colorado/guides/visiting/).

## Getting There and Parking

The prison is on State Highway 96 in Olney Springs, in southeast Colorado.

:::key-info
**By car:** About 54 miles east of Pueblo — U.S. 50 east, then the Highway 96 corridor toward Olney Springs

**By transit:** There is no public transit to the prison; a private vehicle is required
:::

Distances are approximate, based on map routing. Visitor parking is on site.

## Nearby Services

Olney Springs is small; Ordway (a few miles east) has basic services, and La Junta (about 24 miles south) and Pueblo (about 54 miles west) have fuller options. The nearest 24-hour emergency room is **Arkansas Valley Regional Medical Center** in La Junta.

## Mail

Personal mail goes to the facility, addressed with the person's **commitment name and DOC number**. **Crowley County is one of the CDOC facilities that photocopies incoming personal mail, delivers the copy, and destroys the original** — so do not send originals you want returned. **Legal mail** is opened only to inspect for contraband, in the person's presence. Full rules are in [Mail & Packages](/states/colorado/guides/mail/).

## Learn More

For detailed information about visiting and communicating with someone in a Colorado state prison:

- [Visiting in Colorado](/states/colorado/guides/visiting/) — the approved list, the dress code, and ID rules
- [Mail & Packages](/states/colorado/guides/mail/) — the DOC number, photocopying, and publisher-only books
- [Phone & Video Calls](/states/colorado/guides/phone-video/) — free calls, Securus, and tablets
- [Sending Money](/states/colorado/guides/money/) — CorrectPay deposits
- [Medical & Mental Health](/states/colorado/guides/medical/) — the co-pay, HIPAA releases, and Constituent Services
- [Transfers & Finding Someone](/states/colorado/guides/transfers/) — reception, the locator, and VINE
