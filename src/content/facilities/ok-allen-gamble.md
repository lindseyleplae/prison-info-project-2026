---
title: "Allen Gamble Correctional Center"
slug: allen-gamble
state: ok
system: "Oklahoma DOC"
population: men
summary: "A medium-security state prison for men in Holdenville, Oklahoma — the former Davis Correctional Facility, state-run since October 1, 2023."
aliases: ["AGCC", "Allen Gamble", "Allen Gamble Correctional Center", "Davis Correctional Facility"]
city: "Holdenville"
county: "Hughes"
address: "6888 E 133 Road, Holdenville, OK 74848"
securityLevel: "Medium security (men)"
facilityType: "State Prison"
contact:
  mainPhone: "405-698-5900"
  mailingAddress: "6888 E 133 Road, Holdenville, OK 74848"
providers:
  phone: "Securus"
  video: "Securus"
visiting:
  days: "Saturday and Sunday (additional weekdays vary)"
  hours: "8 a.m. – 4 p.m."
  scheduling: "Every visitor must first be on the inmate's approved visitor list (submit a Visitor Application Form and pass a background check; approval can take up to 90 days). Visits are then scheduled through the statewide Visitation Unit at 405-768-3269. All visits must be scheduled in advance — no walk-ins — with requests received by Wednesday at 1 p.m."
  contactVisits: true
links:
  official: "https://oklahoma.gov/doc/facilities.html"
  inmateLocator: "https://okoffender.doc.ok.gov/"
sources:
  - name: "Oklahoma DOC — Facilities"
    url: "https://oklahoma.gov/doc/facilities.html"
  - name: "Oklahoma DOC — Visitation"
    url: "https://oklahoma.gov/doc/facilities/visitation.html"
lastVerified: 2026-06-16
reviewBy: 2026-09-18
---

:::callout{variant="warning"}
Allen Gamble Correctional Center is a medium-security prison for men. This facility was formerly the Davis Correctional Facility, a privately operated CoreCivic prison; the state took over operations and renamed it on October 1, 2023, so a person or document that still uses the old name is referring to this same prison. Confirm the person's current facility on the [Oklahoma DOC offender lookup](https://okoffender.doc.ok.gov/) before traveling or sending anything — people are transferred without family notification. Personal mail does not go to the prison; it goes off-site to a Securus Digital Mail Center for scanning (see [Mail](#mail)).
:::

:::key-info
- **Location:** Holdenville, Hughes County, in east-central Oklahoma
- **Population:** Men, medium custody
- **Notable:** The former Davis Correctional Facility; state-operated since October 1, 2023, when Oklahoma took it over from CoreCivic and renamed it for Sergeant Joe Allen Gamble Jr.
- **Visitation:** By approved list, scheduled through the statewide Visitation Unit; no walk-ins
:::

## Overview

Allen Gamble Correctional Center is a state prison for men operated by the Oklahoma Department of Corrections. It is in Holdenville, the seat of Hughes County, in east-central Oklahoma. The facility houses men in medium custody and has a capacity of about 1,723.

Oklahoma DOC assigns custody level based on factors that include sentence length, time remaining, and conduct, and a person's custody level can change during incarceration. The custody class and housing unit held at a given prison determine whether visits are contact or non-contact, so families confirm the arrangement that applies to the specific person before a first visit.

## What Makes Allen Gamble Correctional Center Different

Allen Gamble Correctional Center was formerly the Davis Correctional Facility, a privately operated prison run by CoreCivic. The state took over operations on October 1, 2023, and renamed the facility after Sergeant Joe Allen Gamble Jr. It is now state-operated. Because the change is recent, a person, a court document, or an older directory may still use the name "Davis Correctional Facility" — that older name refers to this same prison in the same location. The conversion was part of a broader shift in which Oklahoma moved its remaining contracted prisons to state operation; as of mid-2025, Oklahoma's prison system is fully state-operated, with no privately run state prisons.

## Visiting

The statewide Oklahoma DOC rules above — the approved visitor list, the dress code, ID, and item limits — apply at Allen Gamble Correctional Center. The facility's own arrangements:

:::key-info
- **Approved list first:** Before any visit, each visitor must be on the inmate's approved visitor list. Submit a Visitor Application Form (online or by mail to the Visitation Unit, P.O. Box 11400, Oklahoma City, OK 73136-0400) and pass a background check (required for ages 18 and older); approval can take up to 90 days.
- **Scheduling:** Through the statewide Visitation Unit at 405-768-3269. All visits must be scheduled in advance — there are no walk-ins — with requests received by Wednesday at 1 p.m. for that period.
- **Days and hours:** Saturday and Sunday are available, with additional weekdays varying; sessions run 8 a.m. to 4 p.m. Whether a visit is contact or non-contact depends on the person's security level and housing unit.
- **Main line:** 405-698-5900
:::

## Getting There and Parking

Allen Gamble Correctional Center is at 6888 E 133 Road in Holdenville, in Hughes County in east-central Oklahoma. Holdenville is southeast of Oklahoma City and is reached by way of State Highway 9 and US Highway 270. The nearest large commercial airport is in the Oklahoma City area to the northwest.

Parking is on site. Visitors confirm current entry procedures, the visitor-processing location, and what may be brought onto the grounds with the facility before arriving, because electronic devices and personal items are not permitted inside — visitors may generally bring only identification, a car key, and coins for vending machines.

## Nearby Services

Holdenville has limited lodging and dining, with additional options in the surrounding Hughes County communities and a wider range in the Oklahoma City area to the northwest. Emergency medical care is available locally, with larger hospitals in the Oklahoma City area. Visitors traveling a long distance generally find the most options for fuel, food, and overnight stays along the Interstate 40 corridor to the north and in the Oklahoma City area.

## Mail

Oklahoma DOC routes incoming **personal mail** through an off-site digital vendor. As of September 1, 2024, letters and photos are mailed to the Securus Digital Mail Center in Dallas, where they are scanned and delivered to the inmate as a digital color image on a Securus tablet or kiosk — the original is not delivered. Inmates without device access get a free black-and-white printout. Address personal mail exactly as:

> Inmate's Full Name + ODOC Number
> Securus Digital Mail Center - Okla. Dept. of Corrections
> P.O. Box 223566
> Dallas, TX 75222-3566

Illegible or incorrectly addressed mail may be returned or delayed.

**Legal mail and special correspondence** are *not* sent to the digital center — they go directly to the facility and are handled under the rules for privileged mail. **Publications** (magazines, newspapers, books) also go to the facility and must come directly from the publisher or an approved vendor. **Packages** cannot be sent to the digital mail center and must go to the facility.

## Learn More

- [Visiting in Oklahoma](/states/oklahoma/guides/visiting/) — the approved visitor list, scheduling through the Visitation Unit, dress code, and what you can bring.
- [Mail in Oklahoma](/states/oklahoma/guides/mail/) — how Oklahoma's digital mail scanning works and where to send letters, legal mail, and publications.
- [Phone & Video in Oklahoma](/states/oklahoma/guides/phone-video/) — setting up calls, tablets, and video visits through Securus.
- [Sending Money in Oklahoma](/states/oklahoma/guides/money/) — depositing to a trust account through JPay.
- [Medical Care in Oklahoma](/states/oklahoma/guides/medical/) — how health care works in Oklahoma DOC facilities.
- [Transfers & Reception in Oklahoma](/states/oklahoma/guides/transfers/) — intake at LARC (men) and Mabel Bassett (women) and how people move between facilities.
