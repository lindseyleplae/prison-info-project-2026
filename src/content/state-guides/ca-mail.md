...
-   **Letters:** Standard mail is accepted; address it with the person's name and CDCR number.
-   **Electronic messages:** Sent through the tablet system for a per-message fee that depends on the active vendor (ViaPath or Securus).
-   **Packages:** Cannot be mailed by family directly. They must be ordered through a CDCR-approved vendor.
-   **Quarterly package:** Privilege Group A/B is allowed one 30-pound quarterly package each quarter.
-   **Address check:** Confirm the current institution and mailing address on CIRIS before sending anything.
:::
...
Look up the current institution and its mailing address through the [California Incarcerated Records & Information Search (CIRIS)](https://ciris.mt.cdcr.ca.gov/). An institution's mailing ZIP for incarcerated mail can differ from its street address, so use the mailing address CDCR lists.

### What to keep in mind

-   Incoming mail is screened before delivery.
-   Do not send cash. See [Sending Money](/states/california/guides/money/) for approved deposit methods.
...
Full messaging and tablet details, including the transition timeline, are in [Phone & Video Calls](/states/california/guides/phone-video/).
...
Confirm the incarcerated person's current privilege group and housing before ordering. Quarterly package rules are set in CCR Title 15 §3147, and an order placed for an ineligible person can be returned.
:::
...
California is changing its tablet and messaging vendor during 2026, and package eligibility rules change by privilege group. Confirm the current institution and mailing address on CIRIS, the active messaging vendor and price, and the person's quarterly package eligibility before sending mail, messages, or a package.
:::
